...
class Dispatcher {
	static async handle(message) {
		...
		const pre = await message.channel.send('Generating Image <a:loadinggif:413460390996082689>');
		...
		try {
			...
			console.error(err);
			pre.edit(`An error occured while trying to run **${command.name}.**\`\`\`${err.name}: ${err.message}\`\`\`Please contact: **${message.client.users.get(message.client.ownerId).tag}**`);
		}
	}
}
